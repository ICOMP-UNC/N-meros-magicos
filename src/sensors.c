...
void adc_init(void) {
  ...
  adc_set_sample_time(ADC1, ADC_CHANNEL_TEMP_SENSOR,
                      ADC_SMPR_SMP_55DOT5CYC); /* Set sample time */
...
void dma_init(void) {
  ...
  dma_set_peripheral_address(DMA1, DMA_CHANNEL1,
                             (uint32_t)&ADC_DR(ADC1)); /* ADC data register */
  dma_set_memory_address(DMA1, DMA_CHANNEL1,
                         (uint32_t)adc_buffer); /* Memory buffer */
  dma_set_number_of_data(DMA1, DMA_CHANNEL1,
                         ADC_BUFFER_SIZE); /* Number of data items */
  dma_set_priority(DMA1, DMA_CHANNEL1, DMA_CCR_PL_LOW);
  dma_set_memory_size(DMA1, DMA_CHANNEL1,
                      DMA_CCR_MSIZE_16BIT); /* Memory size: 16 bits */
  dma_set_peripheral_size(DMA1, DMA_CHANNEL1,
                          DMA_CCR_PSIZE_16BIT); /* Peripheral size: 16 bits */
...
uint16_t SENSORS_read_temp_history(char *buffer) {
  ...
  for (uint8_t i = 0; i < size_aux; i++) {
    ...
    len += sprintf(buffer + len, "%d.%d°, ", (int32_t)temp,
                   (int32_t)(temp * 10) % 10) -
           1;
  }
  buffer[len] = '\n';
